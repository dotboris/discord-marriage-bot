...
async function createRelationship (relationshipType, { db }, message, person) {
  ...
  if (timeout) {
    ...
    await relationships.add(db, authorId, personId, relationshipType)
  } else {
    await message.reply('Sorry, they said no')
  }
}

function createRelationshipCommand (relationshipType) {
  return async (...args) => createRelationship(relationshipType, ...args)
}

module.exports.marry = createRelationshipCommand('married')
module.exports.date = createRelationshipCommand('dating')
